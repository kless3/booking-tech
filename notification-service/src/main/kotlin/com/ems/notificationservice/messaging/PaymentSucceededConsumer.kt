...
class PaymentSucceededConsumer(
    ...
    @KafkaListener(topics = ["\${app.kafka.topics.payment-succeeded}"])
    fun handle(record: ConsumerRecord<String, String>) {
        val event = parseEventOrSendToDlt(record) ?: return
        if (event.eventType != PAYMENT_SUCCEEDED_EVENT_TYPE) {
    ...
    private fun parseEventOrSendToDlt(record: ConsumerRecord<String, String>): PaymentSucceededEvent? =
        try {
            objectMapper.readValue(record.value(), PaymentSucceededEvent::class.java)
        } catch (exception: Exception) {
            sendToDlt(record, "Unable to deserialize payment.succeeded event: ${exception.message}")
            null
        }
    ...
    private companion object {
        const val PAYMENT_SUCCEEDED_EVENT_TYPE = "payment.succeeded"
    }
}
